...
@app.post("/sponsorship/create-checkout")
@rate_limit("sponsorship_checkout", 15, 300)
def sponsorship_checkout_create():
    show_slug = request.form.get("show_slug", "").strip()
    show = get_show_by_slug(show_slug)
    if not show: return "Show not found.", 404
    acct = _connected_account_id(show)
    if not acct: return render_template("payment_not_complete.html"), 400

    catalog_id_raw = request.form.get("catalog_id", "").strip()
    if not catalog_id_raw.isdigit(): return render_template("payment_not_complete.html"), 400
    catalog_id = int(catalog_id_raw)
    catalog = get_catalog_item(catalog_id)
    if not catalog or int(catalog["show_id"]) != int(show["id"]): return render_template("payment_not_complete.html"), 404
    ...
    if file_storage and getattr(file_storage, "filename", ""):
        ...
        if filename:
            ...
            if ext in {"png","jpg","jpeg","webp","svg"}:
                ...
                logo_path = f"img/sponsors/uploads/{final_name}"
    ...
    session_obj = stripe.checkout.Session.create(
        ...
        payment_method_types=["card"],
        line_items=[{
            "price_data":{"currency":"usd","unit_amount":int(catalog["price_cents"] or 0),"product_data":{"name":f"Sponsorship – {catalog['package_name']} ({show['title']})"}},
            "quantity":1,
        }],
        success_url=success_url,
        cancel_url=cancel_url,
        customer_email=request.form.get("email","").strip() or None,
        metadata={"payment_item_type":"sponsorship","show_id":str(show["id"]),"show_slug":show_slug,"sale_id":str(sale_id),"catalog_id":str(catalog_id)},
        stripe_account=acct,
    )
