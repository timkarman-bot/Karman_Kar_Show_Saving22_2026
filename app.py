...
def admin_shows():
    ...
    pending_vote_reviews = []
    if active_show:
        pending_vote_reviews = list_pending_vote_reviews(int(active_show["id"]))
    ...
    return render_template(
        ...
        pending_vote_reviews=pending_vote_reviews,
    )

        
@app.post("/admin/votes/<int:vote_intent_id>/approve")
@require_admin
def admin_approve_vote(vote_intent_id: int):
    try:
        finalize_external_vote_intent(
            vote_intent_id,
            approval_reference="admin_approved",
        )
        flash("Vote approved and counted.", "ok")
    except Exception as e:
        flash(f"Could not approve vote: {e}", "error")
    return redirect(url_for("admin_shows"))

@app.post("/admin/votes/<int:vote_intent_id>/reject")
@require_admin
def admin_reject_vote(vote_intent_id: int):
    try:
        reject_external_vote_intent(
            vote_intent_id,
            rejection_reference="admin_rejected",
        )
        flash("Vote marked as rejected.", "ok")
    except Exception as e:
...
def admin_shows():
    ...
    pending_vote_reviews = list_pending_vote_reviews(
        int(active_show["id"]) if active_show else None
    )
